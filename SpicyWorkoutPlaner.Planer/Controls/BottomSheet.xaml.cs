...
public partial class BottomSheet : ContentView
{
    ...
    public async Task OpenSheet()
    {
        try
        {
            await Task.WhenAll
            (
                Background.FadeTo(0.4, length: duration),
                Sheet.TranslateTo(0, openPosition, length: duration, easing: Easing.SinIn)
            );

            BottomSheetRef.InputTransparent = Background.InputTransparent = false;
        }
        catch (Exception ex)
        {
            //ex.Log();
    ...
    public async Task CloseSheet()
    {
        try
        {
            await Task.WhenAll
            (
                Background.FadeTo(0, length: duration),
                PanContainerRef.Content.TranslateTo(x: 0, y: SheetHeight + 60, length: duration, easing: Easing.SinIn)
            );

            BottomSheetRef.InputTransparent = Background.InputTransparent = true;
        }
        catch (Exception ex)
        {
            //ex.Log();
